...
class Container {
  ...
  async plantShapeTreeInstance (shapeTreeUrl, requestedName, payloadGraph) {
    ...
    const funcDetails = Details.extend(`plantShapeTreeInstance(<${shapeTreeUrl.href}>), Container(<${this.url.pathname}>), "${requestedName}", Store() with ${payloadGraph.size} quads)`);
    funcDetails('');
    let location;
