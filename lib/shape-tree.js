...
class Container {
  ...
  async nestContainer (slug, title) {
    const prefixes = {};
    const [name, graph] = await fileSystem.inventContainer(this.url, slug, `unmanaged Container in ${this.url.href}`, prefixes);
    console.assert(name.match(/^[^/]+\/$/));
    const url = new URL(name, this.url);
    const ret = new Container(url, [graph, prefixes]); // kinda inefficient
    ret.graph.removeQuads(namedNode(url.href), namedNode(Prefixes.ns_dc + 'title'), null);
    ret.graph.addQuad(namedNode(url.href), namedNode(Prefixes.ns_dc + 'title'),
                      literal(`unmanaged Container ${url.pathname}`));
    await ret.write();
    return ret;
  }
...
class ManagedContainer extends Container {
  ...
  async nestContainer (slug, title) {
    const prefixes = {};
    const [name, graph] = await fileSystem.inventContainer(this.url, slug, `managed Container in ${this.url.href}`, prefixes);
    console.assert(name.match(/^[^/]+\/$/));
    const url = new URL(name, this.url);
    const newInstancePath = this.shapeTreeInstancePath + name;
    const ret = await new Container(url, [graph, prefixes]).asManagedContainer(this.shapeTreeUrl, newInstancePath); // kinda inefficient
    ret.graph.removeQuads(namedNode(url.href), namedNode(Prefixes.ns_dc + 'title'), null);
    ret.graph.addQuad(namedNode(url.href), namedNode(Prefixes.ns_dc + 'title'),
                      literal(`Container for nested resource ${newInstancePath}`));
    await ret.write();
    return ret;
  }
  ...
  async instantiateStatic (stepNode, rootUrl, pathWithinShapeTree, parent, container = null) {
    ...
    try {
      ...
      const subdirPromises = contents.reduce((acc, nested) => {
        ...
        return acc.concat(step.instantiateStatic(nested, new URL(Path.join(toAdd, '/'), rootUrl), step.path, ret));
      }, []);
