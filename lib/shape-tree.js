...
class Container {
  constructor (url, title) {
    ...
    this._classDetails = Details.extend(`Container(<${url.pathname}>, ${title instanceof Array ? `[Store with ${title[0].size} quads, ${JSON.stringify(title[1])}]` : `"${title}"`})`);
    ...
    async function loadOrCreate () {
      ...
      funcDetails('fileSystem.ensureContainer()', url.pathname);
  ...
  async nest (slug, payload, mediaType) {
    ...
    funcDetails('fileSystem.invent(<%s>, "%s", %d characters, "%s")', this.url.pathname, slug, payload.length, mediaType);
    const [name] = await fileSystem.invent(this.url, slug, payload, mediaType);
  ...
  async nestContainer (slug, title) {
    ...
    funcDetails('fileSystem.inventContainer(%s, %s, %s, %s)', this.url.pathname, slug, containerTitle, JSON.stringify(prefixes));
    const [name, graph] = await fileSystem.inventContainer(this.url, slug, containerTitle, prefixes);
    console.assert(name.match(/^[^/]+\/$/));
    const url = new URL(name, this.url);
  ...
  async write () {
    ...
    await fileSystem.writeContainer(this.url, this.graph, this.prefixes).then(
      x => { unlock(); return x; },
...
class ManagedContainer extends Container {
  constructor (url, title, shapeTreeUrl, shapeTreeInstancePath) {
    super(url, title);
    this._classDetails = Details.extend(`ManagedContainer(<${url.pathname}>, ${title instanceof Array ? `[Store with ${title[0].size} quads, ${JSON.stringify(title[1])}]` : `"${title}", <${shapeTreeUrl.pathname}>, "${shapeTreeInstancePath}"`})`);
    const funcDetails = this._classDetails.extend('new');
    funcDetails('');
    ...
    async function loadOrCreate () {
      const unlock = await this._mutex.lock();
      if (this.newDir) {
        funcDetails('this.asManagedContainer(<%s>, "%s")', shapeTreeUrl. shapeTreeInstancePath);
        await this.asManagedContainer(shapeTreeUrl, shapeTreeInstancePath);
        funcDetails('fileSystem.writeContainer(<%s>, Store() with %d quads, %s)', this.url.pathname, this.graph.size, JSON.stringify(this.prefixes));
        await fileSystem.writeContainer(this.url, this.graph, this.prefixes);
      } else {
        parseShapeTreeInstance.call(this);
      }
      unlock();
  ...
  async nestContainer (slug, title) {
    ...
    funcDetails('new Container(%s, [...]).asManagedContainer(%s, %s)', this.url.pathname, this.shapeTreeUrl, newInstancePath);
...
class RemoteResource {
  constructor (url) {
    ...
    this._classDetails = Details.extend(`RemoteResource(<${url.pathname}>)`);
  ...
  async fetch () {
    this._classDetails('fetch()', this.url.pathname);
...
class RemoteShapeTree extends RemoteResource {
  constructor (url, path = '.') {
    super(url);
    this._classDetails = Details.extend(`RemoteShapeTree(<${url.pathname}>, "${path}")`);
    this._classDetails('new')
    this.path = path
  }
  ...
  async instantiateStatic (stepNode, rootUrl, pathWithinShapeTree, parent, container = null) {
    ...
    if (container) {
      ...
      funcDetails('new ManagedContainer(<%s>, "%s", <%s>, "%s")', rootUrl.pathname, containerTitle, this.url.pathname, pathWithinShapeTree);
    ...
    try {
      ...
      const subdirPromises = contents.reduce((acc, nested) => {
        ...
        funcDetails('new RemoteShapeTree(<%s>, "%s")', this.url.pathname, Path.join(pathWithinShapeTree, toAdd, '/'));
