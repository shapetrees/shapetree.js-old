...
class RemoteShapeTree extends RemoteResource {
  ...
  async walkReferencedTrees () {
    ...
    async function walkList (stepNode) {
      const references = _RemoteShapeTree.graph.getQuads(stepNode, namedNode(Prefixes.ns_tree + 'references'), null)
            .map(ref => ({
              step: _RemoteShapeTree.graph.getQuads(ref.object, namedNode(Prefixes.ns_tree + 'step'), null)[0].object,
              path: _RemoteShapeTree.graph.getQuads(ref.object, namedNode(Prefixes.ns_tree + 'path'), null)[0].object.value
            }));
      const iterators = await Promise.all(references.map(
        async r => (await (new RemoteShapeTree(
          new URL(r.step.value)
        )).fetch())
          .walkReferencedTrees()
      ));
      const list =
            iterators.map(e => [...e]); // expand each iterator
            // await Promise.all(references.map(r => walkList(r.step))) // only works for same url

      return references.map(
        r => ({step: new URL(r.step.value), path: r.path})
      ).concat(
        list
          .flat()
      );
