...
class Container {
  ...
  async asManagedContainer (shapeTreeUrl, shapeTreeInstancePath) {
    const mdGraph = await mergeTreeMetadata(this.url, shapeTreeUrl, shapeTreeInstancePath, this.prefixes);
  ...
  async function mergeTreeMetadata (describedUrl, shapeTreeUrl, shapeTreeInstancePath, prefixes) {
...
<${fromMd}>
   ...
   tree:shapeTreeInstanceRoot <${Path.relative(instanceRootRel, '')}> .
`;
...
class ManagedContainer extends Container {
  ...
  async nest (slug, payload, mediaType) {
    ...
    await mergeTreeMetadata(url, this.shapeTreeUrl, pathAppend(this.shapeTreeInstancePath, added), {});
    return url;
  }
  ...
  async parse () {
    ...
    for (const key in subjects) {
      try {
        ...
        debugger; // non-ShapeTree node in the ShapeTree graph. I guess that's OK.
      }
    }
...
RemoteShapeTree.DEFAULT = 0xE
  ...
  async function* walkReferencedResources (focus, control = RemoteShapeTree.DEFAULT, via = []) {
    ...
    Log.extend('ShapeTree')('validate graph (%d triples) with ShapeMap <%s>@<%s>', dataGraph.size, focus.href, shape);
    let res = v.validate(ShExCore.Util.makeN3DB(dataGraph), focus.href, shape);
  ...
  async function getMetaData (focus, prefixes) {
    ...
    return {
      ...
      validationFocus: new URL(rdfInterface.one(g, s, namedNode(Prefixes.ns_tree + 'validationFocus'), null).object.value),
    }
  }
