...
`;
    ...
    storage.writeMetaData(this.url, mdGraph, {tree: Prefixes.ns_tree});
...
class RemoteShapeTree extends RemoteResource {
  ...
  async parse () {
    ...
    const contents = namedNode(Prefixes.ns_tree + 'contents')
    ...
    const stepRules = [
      { predicate: Prefixes.ns_tree + 'expectedType', attr: 'expectedType', f: sht },
      { predicate: Prefixes.ns_rdfs + 'label'       , attr: 'name'        , f: str },
      { predicate: Prefixes.ns_tree + 'uriTemplate' , attr: 'uriTemplate' , f: str },
      { predicate: Prefixes.ns_tree + 'shape'       , attr: 'shape'       , f: sht },
      { predicate: Prefixes.ns_tree + 'contents'    , attr: 'contents'    , f: cnt },
    ...
    function visitStep (rules, subjects, includeId) {debugger;
  ...
  async walkReferencedTrees999 () {
    const _RemoteShapeTree = this;
    let node = namedNode(this.url.href);
    const iz = await walkList(node);
    const ret = {hash: this.url.hash, iz: iz};

    ret[Symbol.iterator] = function () {
      let idx = 0;
      return {
        next: function () {
          return idx < iz.length ? {
            value: iz[idx++],
            done: false
          } : {
            done: true
    ...
    async function walkList (stepNode) {
      const g = _RemoteShapeTree.graph;
      const references = g.getQuads(stepNode, namedNode(Prefixes.ns_tree + 'references'), null)
            .map(ref => ({
              treeStep: g.getQuads(ref.object, namedNode(Prefixes.ns_tree + 'treeStep'), null)[0].object,
              shapePath: g.getQuads(ref.object, namedNode(Prefixes.ns_tree + 'shapePath'), null)[0].object.value,
              // referrers: [new URL(stepNode.value)]
            }));
      const iterables = await Promise.all(references.map(
        // cross document boundaries to fetch steps.
        async r => (await (new RemoteShapeTree(
          new URL(r.treeStep.value)
        )).fetch())
          .walkReferencedTrees()
      ));
      const list =
            // expand each iterable (@@inefficient)
            iterables.map(i => [...i]);
            // await Promise.all(references.map(r => walkList(r.treeStep))) // doesn't cross document boundaries

      return references
        .map(r => ({treeStep: new URL(r.treeStep.value), shapePath: r.shapePath}))
        .concat(list.flat());
    }
  }
}
