...
class Container {
   ...
   */
  async nestContainer (url, title) {
    console.assert(url.href.startsWith(this.url.href));
    console.assert(url.pathname.substr(this.url.pathname.length).match(/^[^/]+\/$/));
    return new Container(url, `index for unmanaged Container ${url.pathname}`);
  }
...
class ManagedContainer extends Container {
  ...
  async nestContainer (url, title) {
    console.assert(url.href.startsWith(this.url.href));
    const added = url.pathname.substr(this.url.pathname.length);
    console.assert(added.match(/^[^/]+\/$/));
    const newInstancePath = this.shapeTreeInstancePath + added;
    return new ManagedContainer(url, `index for nested resource ${newInstancePath}`, this.shapeTreeUrl, newInstancePath);
  }
...
class RemoteShapeTree extends RemoteResource {
  ...
  async instantiateStatic (stepNode, rootUrl, pathWithinShapeTree, parent, container = null) {
    let ret;
    if (container) {
      ret = container; // @@ await container.asManagedContainer(this.url, pathWithinShapeTree) and remove from plantShapeTreeInstance
    } else {
      ret = await new ManagedContainer(rootUrl,
                                       `index for nested resource ${pathWithinShapeTree}`, // @@ change to 'container for...'
                                       this.url, pathWithinShapeTree).ready;
      parent.addMember(ret.url.href, stepNode.url);
    }

    try {
      ret.addSubdirs(await Promise.all(this.graph.getQuads(stepNode, Prefixes.ns_tree + 'contents', null).map(async q => {
        const nested = q.object;
        ...
        step.graph = this.graph; // no need to fetch it again
