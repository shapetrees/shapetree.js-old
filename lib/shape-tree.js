...
class Container {
  ...
  async nestContainer (slug, title) {
    const prefixes = {};
    const [name, graph] = await fileSystem.inventContainer(this.url, slug, `unmanaged Container in ${this.url.pathname}`, prefixes);
    console.assert(name.match(/^[^/]+\/$/));
    const url = new URL(name, this.url);
    const ret = new Container(url, [graph, prefixes]); // kinda inefficient
    ret.setTitle(`index for unmanaged Container ${url.pathname}`); // @@ s/index for //
    await ret.write();
    return ret;
  }
...
class ManagedContainer extends Container {
  ...
  async nestContainer (slug, title) {
    const prefixes = {};
    const [name, graph] = await fileSystem.inventContainer(this.url, slug, `managed Container in ${this.url.pathname}`, prefixes);
    console.assert(name.match(/^[^/]+\/$/));
    const url = new URL(name, this.url);
    ...
    ret.setTitle(`index for nested resource ${newInstancePath}`); // @@ s/index/Container/
