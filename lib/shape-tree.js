...
const ShExPath = require('../../../../shexSpec/shex.js/packages/shape-path')
const ShExParser = require('@shexjs/parser')

// Debugging controls
//   comparator for ordering e.g. ShapeTree references
const CMP = (l, r) => l.localeCompare(r) // undefine if you want no sorting
...
class Container {
  ...
  setTitle (title) {
    this.graph.getQuads(
      namedNode(this.url.href), namedNode(Prefixes.dc + 'title'), null
    ).map(
      q => this.graph.removeQuad(q)
    );
    this.graph.addQuad(namedNode(this.url.href), namedNode(Prefixes.dc + 'title'),
                       literal(title));
  }
  ...
  addMember (location, shapeTreeUrl) {
    this.graph.addQuad(namedNode(this.url.href), namedNode(Prefixes.ldp + 'contains'), namedNode(location));
    return this
  }

  removeMember (location, shapeTreeUrl) {
    this.graph.removeQuad(namedNode(this.url.href), namedNode(Prefixes.ldp + 'contains'), namedNode(location));
    return this
  }
  ...
  async function mergeTreeMetadata (describedUrl, shapeTreeUrl, shapeTreeInstancePath, validationRoot, prefixes) {
   ...
   tree:validationRoot <${validationRoot.href}>`
    ...
    (['shapeTreeRoot', 'shapeTreeInstancePath', 'shapeTreeInstanceRoot'])
      .forEach(
        ln =>
          mdGraph.removeQuads(mdGraph.getQuads(mdNode, namedNode(Prefixes.tree + ln), null))
      )
    const c = `
@prefix dcterms: <http://purl.org/dc/terms/>.
@prefix ldp: <http://www.w3.org/ns/ldp#>.
@prefix tree: <${Prefixes.tree}>.
...
`;
    ...
    storage.writeMetaData(describedUrl, mdGraph, {tree: Prefixes.tree});
    return mdGraph;
  }
...
class ManagedContainer extends Container {
  ...
  async getRootedShapeTree () {
    ...
    const path = rdfInterface.one(mdGraph, namedNode(this.url.href), namedNode(Prefixes.tree + 'shapeTreeInstancePath'), null).object.value;
    const root = rdfInterface.one(mdGraph, namedNode(this.url.href), namedNode(Prefixes.tree + 'shapeTreeRoot'), null).object.value;
...
function asUrl (g, s, p) {
  const ret = rdfInterface.zeroOrOne(g, namedNode(s.href), namedNode(Prefixes.tree + p), null);
...
class RemoteShapeTree extends RemoteResource {
  ...
  async parse () {
    ...
    const contains = namedNode(Prefixes.tree + 'contains')
    const containedInMap = {}
    ...
    const cnt = (sz, g, p) => {
      return sz.map(r => {
        const child = new URL(r.value)
        containedInMap[child.href] = new URL(p.value)
        return child
      })
    }
    const ref = sz => visitNode(_RemoteShapeTree.graph, referenceRules, sz, null)
    ...
    const referenceRules = [
      ...
      { predicate: Prefixes.tree + 'shapePath'   , attr: 'shapePath'   , f: str },
    ]
    const stepRules = [
      { predicate: Prefixes.rdfs + 'label'       , attr: 'name'        , f: str },

      // predicates with a domain of tree:ShapeTree :
      { predicate: Prefixes.tree + 'expectsType' , attr: 'expectsType' , f: url },
      { predicate: Prefixes.tree + 'matchesUriTemplate' , attr: 'matchesUriTemplate' , f: str },
      { predicate: Prefixes.tree + 'validatedBy' , attr: 'validatedBy' , f: url },
      { predicate: Prefixes.tree + 'supports'    , attr: 'supports'    , f: lst },
      { predicate: Prefixes.tree + 'contains'    , attr: 'contains'    , f: cnt },
      { predicate: Prefixes.tree + 'references'  , attr: 'references'  , f: ref },
    ]

    // Look for subjects with any predicate with a domain of tree:ShapeTree.
    // @@ISSUE: What are the recognition rules for ShapeTree steps?
    const subjects = stepRules.slice(1).reduce(
      (outer, rule) =>
        _RemoteShapeTree.graph.getQuads(null, namedNode(rule.predicate), null).reduce((inner, q) => {
    ...
    // Parse each of those subjects against stepRules
    for (const s in subjects) {
      // try {
        const step = visitNode(_RemoteShapeTree.graph, stepRules, [subjects[s]], '@id')[0]
        if (typeof CMP !== 'undefined' && 'references' in step)
          step.references = step.references.sort((l, r) => CMP(l.treeStep.href, r.treeStep.href))
    ...
    // Add parent links to any contained steps *in this document*.
    for (const child in containedInMap)
      // @@issue: should parse have automatically loaded out-of-doc comtains?
      // if (!(child in _RemoteShapeTree.ids))
      // e.g. if deep analog to <gh-flat-ShapeTree-split-org#org> contained <gh-deep-ShapeTree-split-issues#issues>
      _RemoteShapeTree.ids[child].containedIn = containedInMap[child]

    // A decorator index arc will come from the document URL.
    const decIdxPredicate = namedNode(Prefixes.tree + 'hasShapeTreeDecoratorIndex')
    const indexes =
          this.graph.getQuads(namedNode(noHash(this.url).href), decIdxPredicate, null)
  ...
  getRdfRoot () {
    return this.path.split(/\//).reduce((node, name) => {
      ...
      const cqz = this.graph.getQuads(node, namedNode(Prefixes.tree + 'contains'), null);
      // Find the element which either
      return cqz.find(
        q =>
          // matches the current label in the path
        this.graph.getQuads(q.object, namedNode(Prefixes.rdfs + 'label'), literal(name)).length === 1
          ||
          // or has a matchesUriTemplate (so it should be the sole element in the contents)
        this.graph.getQuads(q.object, namedNode(Prefixes.tree + 'matchesUriTemplate'), null).length === 1
      ).object
  ...
  matchingStep (shapeTreeNode, slug) {
    const contains = this.graph.getQuads(shapeTreeNode, namedNode(Prefixes.tree + 'contains'))
          .map(q => q.object);
    const choices = contains
          .filter(
            step => !slug ||
              new UriTemplate(
                this.graph.getQuads(step, namedNode(Prefixes.tree + 'matchesUriTemplate'))
    ...
    const ret = {
      ...
      contains: this.graph.getQuads(choices[0], Prefixes.tree + 'contains', null).map(t => t.object)
    };
    /* istanbul ignore else */ if (typeNode)
      ret.type = typeNode.value.replace(Prefixes.ldp, '');
    return ret;

    function obj (property) {
      const q = rdfInterface.zeroOrOne(g, choices[0], namedNode(Prefixes.tree + property), null);
  ...
  async instantiateStatic (stepNode, rootUrl, pathWithinShapeTree, parent, container = null) {
    ...
    try {
      const contains = this.graph.getQuads(stepNode, Prefixes.tree + 'contains', null).map(q => q.object);
      const subdirPromises = contains.reduce((acc, nested) => {
        const labelQ = rdfInterface.zeroOrOne(this.graph, nested, namedNode(Prefixes.rdfs + 'label'), null);
        if (!labelQ)
   ...
   * @returns {Iterable}
   */
  async *walkReferencedTrees (control = RemoteShapeTree.DEFAULT, via = []) {
    const _RemoteShapeTree = this
...
RemoteShapeTree.DEFAULT = 0xE
  ...
  async function getMetaData (focus, prefixes) {
    ...
    return {
      shapeTreeRoot: new URL(rdfInterface.one(g, s, namedNode(Prefixes.tree + 'shapeTreeRoot'), null).object.value),
      shapeTreeInstancePath: rdfInterface.one(g, s, namedNode(Prefixes.tree + 'shapeTreeInstancePath'), null).object.value,
      shapeTreeInstanceRoot: new URL(rdfInterface.one(g, s, namedNode(Prefixes.tree + 'shapeTreeInstanceRoot'), null).object.value),
      validationFocus: new URL(rdfInterface.one(g, s, namedNode(Prefixes.tree + 'validationRoot'), null).object.value),
    }
  }
