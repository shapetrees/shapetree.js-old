...
class Container {
  ...
  async nestContainer (url, title) {
    ...
    return new Container(url, `index for unmanaged Container ${url.pathname}`).ready;
  }
...
class ManagedContainer extends Container {
  ...
  async nestContainer (url, title) {
    ...
    return new ManagedContainer(url, `index for nested resource ${newInstancePath}`, this.shapeTreeUrl, newInstancePath).ready;
  }
