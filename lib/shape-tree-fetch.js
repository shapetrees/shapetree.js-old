      ...
      case 'POST': {
        ...
        if (isPlantRequest) {
          ...
          if (location) {
            ...
            funcDetails(`ecosystem.plantShapeTreeInstance(<${shapeTreeUrl.href}>, parentContainer(<${parentContainer.url.pathname}>), "${requestedName.replace(/\/$/, '')}", n3.Store() with ${payloadGraph.size} quads)`);
          ...
          try {
            if (ldpType === 'Container') {
              ...
              await created.asManagedContainer(parentContainer.shapeTreeUrl, newShapeTreeInstancePath)
              await created.write();
            }
          } catch (e) {
