        ...
        if (isPlantRequest) {
          ...
          try {
            if (ldpType === 'Container') {
              ...
              location = new URL(ret.headers.get('location'));
              const created = await ShapeTree.loadContainer(location);
              await dirMaker(created);
              const prefixes = {};
              const c = containerText(null, postedContainer.shapeTreeUrl, postedContainer.shapeTreeInstancePath + '/' + toAdd.replace(/\/$/, ''), created.prefixes);
              const s = await rdfInterface.parseTurtle(c, location, created.prefixes);
              created.graph.addQuads(s.getQuads());
              await created.write();
...
async function validatePost (location, payload, headers, ldpType, entityUrl, postedContainer, toAdd) {
  ...
  return [payloadGraph, async (serversNewContainer) => {
    const dir = await shapeTree.instantiateStatic(step.node, location, pathWithinShapeTree, postedContainer, serversNewContainer)
